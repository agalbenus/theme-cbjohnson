...
function __custom_cmd_duration -S -d 'Show command duration'
    ...
    if [ "$CMD_DURATION" -lt 5000 ]
        ...
        __custom_pretty_ms $CMD_DURATION s
    else if [ "$CMD_DURATION" -lt 3600000 ]
        set_color $fish_color_error
        __custom_pretty_ms $CMD_DURATION m
    else
        set_color $fish_color_error
        __custom_pretty_ms $CMD_DURATION h
    end
    ...
    or echo -ns ' ' $__custom_left_arrow_glyph
end

function __custom_pretty_ms -S -a ms -a interval -d 'Millisecond formatting for humans'
    set -l interval_ms
    set -l scale 1
...
function __custom_timestamp -S -d 'Show the current timestamp'
    [ "$theme_display_date" = "no" ]
    and return
...
function fish_right_prompt -d 'is all about the right prompt'
    set -l __custom_left_arrow_glyph '<'
    set_color $fish_color_autosuggestion

    __custom_cmd_duration
    __custom_timestamp
    set_color normal
end
